...
private class MyFluidData implements DwFluid2D.FluidData {
  ...
  @Override
    ...
    //particles.spawn(fluid, px, py, radius, 100);
     ...
     // Loop through the vertices of the SVG shape
...
//SVG stuff
PImage ahmad;
PShape svg; // Declare a PShape object to hold the loaded SVG.
PGraphics svgPg;
PGraphics pg; // Declare a PGraphics object for drawing the SVG.
...
public void setup() {
  ...
  // Copy the SVG PGraphics into pg_obstacles
    pg_obstacles.shape(svg, 0, 0);
  //pg_obstacles.image(ahmad, 0, 0);
...
print(contours.size() + "\n");
  if (contours.size() > 0) {
    for (Contour contour : contours) {
      ...
      if (contour.numPoints() > 50) {
        ...
        stroke(0, 200, 200);
        svg.beginShape();
        int boundaryPadding = 80;
        for (PVector point : contour.getPolygonApproximation ().getPoints()) {
          if(point.x > boundaryPadding && point.x < 512-boundaryPadding && point.y > boundaryPadding && point.y < height-boundaryPadding)
        ...
        noFill();
        stroke(200,200,0);
        vertex(boundaryPadding,boundaryPadding);
        vertex(512-boundaryPadding, boundaryPadding);
        vertex(512-boundaryPadding, 512-boundaryPadding);
        vertex(boundaryPadding, 512-boundaryPadding);
        endShape(CLOSE);
      }
    }
  }
  ...
  // Copy the SVG PGraphics into pg_obstacles
  pg_obstacles.shape(svg,0, 0);
  //pg_obstacles.image(kinect.getDepthImage(), mouseX, 0);
